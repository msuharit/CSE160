...
uniform float u_texColorWeight; // 1.0 = texture, 0.0 = color
...
image.onload = function () {
  ...
  g_texture = texture;
  callback(); // ensure tick() + renderAllShapes() only run here
};
...
function renderAllShapes() {
  ...
  const wall = new Cube();
  wall.matrix.setTranslate(-5, -5, -5);
  wall.matrix.scale(10, 10, 10);
  wall.color = [1.0, 1.0, 1.0, 1.0];
  wall.textureEnabled = true;
  wall.render();
